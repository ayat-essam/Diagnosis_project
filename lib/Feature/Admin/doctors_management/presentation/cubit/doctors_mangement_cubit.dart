...
class DoctorsManagementCubit extends Cubit<DoctorsManagementState> {
  ...
  final DeactivateDoctorUsecase deactivateDoctorUsecase;

  DoctorsManagementCubit(
      {required this.getDoctorsUseCase, required this.getDoctorProfileUseCase, required this.deactivateDoctorUsecase})
  ...
  void deaviteDoctor(int id) async {
    emit(DeactivateDoctorLoading());

    final result = await deactivateDoctorUsecase(id: id);

    result.fold(
      (l) => emit(DeactivateDoctorError(l.errorMessage)),
      (r) => emit(DeactivateDoctorSuccess()),
    );
  }
